...
# update an existing item table row
def put_item(connection, item_id):
    # get data and required debugging statements
    request_data = request.get_json()
    user_data = request_data.keys()
    failed_data_entry_statement = "put_item error. check terminal"
    terminal_error_statement = "put_item error:"

    print("\nprocessing put_item...")
    # If no keys and values are provided in the body in POSTMAN and throw error if PK is provided
    verify_user_data = verify_initial_data("item", "ITEM_ID", user_data, terminal_error_statement, update_entity=True, pk_value=item_id)
    ...
    required_attributes, allowed_attributes = entity_attributes_provider("item")
    ...
    try:
        ...
        validate_attribute("ITEM_QTY", request_data, user_attributes_names, user_attributes_values, vartype=int, max=9999)
        validate_attribute("ROOM_NUM", request_data, user_attributes_names, user_attributes_values, vartype=int, max=999)
        ...
        validate_attribute("INVT_CODE", request_data, user_attributes_names, user_attributes_values, vartype=int, isforeignkey=True, foreign_entity_name="inventory", foreign_key_name="INVT_CODE")
    ...
    query = update_query("item", user_attributes_names, user_attributes_values, "ITEM_ID", item_id)
    print(f"{query}")
    ...
    print("put item success")
    return "put item success"