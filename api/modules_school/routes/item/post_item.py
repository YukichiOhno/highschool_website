...
def post_item(connection):
    # get data and required debugging statements
    request_data = request.get_json()
    user_data = request_data.keys()
    failed_data_entry_statement = "post_item error. check terminal"
    terminal_error_statement = "post_item error:"
    
    print("\nprocessing post_item...")
    # If no keys and values are provided in the body in POSTMAN and throw error if PK is provided
    verify_user_data = verify_initial_data("item", "ITEM_ID", user_data, terminal_error_statement)
    ...
    query = insert_query("item", user_attributes_names, user_attributes_values)
    print(f"{query}")
